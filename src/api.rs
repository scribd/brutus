...
pub async fn vector_search(mut req: Request<()>) -> Result<Body> {
    ...
    .iter()
    .map(|n|{ ChunkResponse{chunk: n.0.idx().unwrap(),
        score: n.1,
    ...
    for (score, doc_address) in top_docs {
        ...
        result.push(
            ChunkResponse {
                ...
                score: score as f64,
                text: retrieved_doc.get_first(text).unwrap().as_text().unwrap().to_string()
            }
        );
    }
...
mod tests {
    ...
    fn test_nn_search() {
        ...
        assert!(nn_search(&samples, seed).len() > 0);
